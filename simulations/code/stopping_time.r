...
g=ggplot(df,aes(x=time,y=S,group=nwalk,color=nwalk))+geom_line(show.legend=F)
ggplotly(g)
...
df$nwalk<-as.factor(df$nwalk)
g=ggplot(df,aes(x=time,y=S,group=nwalk,color=nwalk))+geom_line(show.legend=F)
g=g+geom_hline(yintercept =0)+geom_segment(aes(x=0,y=0,xend=N,yend=N),show.legend=F,color="black",linetype=2)+geom_segment(aes(x=0,y=0,xend=N,yend=-N),show.legend=F,color="black",linetype=2)
ggplotly(g)
...
ggplot(df,aes(x=nsim,y=sample_mean))+geom_line()+geom_line(aes(x=nsim,y=ref))
...
g=ggplot(df,aes(x=S_T))+geom_histogram(binwidth = 2)+xlim(-20,20)+geom_freqpoly(binwidth=2)+labs(title ="Histogram for S_T")
ggplotly(g)